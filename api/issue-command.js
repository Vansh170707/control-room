...
export default async function handler(request, response) {
  ...
  if (normalizeSecret(clientSecret) !== normalizeSecret(ingestSecret)) {
    json(response, 401, {
      error:
        "Cloud bridge owner secret is required before terminal commands can be queued.",
    });
    return;
  }
  ...
  if (!agentId || !command) {
    json(response, 400, { error: "agentId and command are required" });
    return;
  }

  const functionUrl = `${supabaseUrl.replace(/\/$/, "")}/functions/v1/issue-command`;
