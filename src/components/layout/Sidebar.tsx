...
import { Plus, Settings2, ScrollText, Server } from "lucide-react";
...
export function Sidebar({
  ...
  return (
    <aside
      className="relative z-10 flex h-[100dvh] min-h-0 shrink-0 flex-col overflow-hidden border-r border-white/[0.07] bg-[linear-gradient(180deg,#131923_0%,#10161f_100%)] shadow-[inset_-1px_0_0_rgba(255,255,255,0.02)]"
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected
                      ? "border border-[#2f4a67] bg-[linear-gradient(90deg,rgba(59,130,246,0.18),rgba(59,130,246,0.04))] text-[#f2f7fb] shadow-[0_0_0_1px_rgba(59,130,246,0.12)]"
                      : "border border-transparent text-[#c9d1d9] hover:border-white/[0.06] hover:bg-[#1a212b]",
                  )}
                >
                  <div
                    className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-md text-[13px]"
              ...
              channels.map((channel) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex w-full items-center gap-2 rounded-lg border px-2.5 py-2 text-left transition-all duration-150",
                      isSelected
                        ? "border-[#2f4a67] bg-[linear-gradient(90deg,rgba(59,130,246,0.16),rgba(59,130,246,0.03))] text-[#eef5fc]"
                        : "border-transparent text-[#c9d1d9] hover:border-white/[0.06] hover:bg-[#1a212b]",
                    )}
                  >
        ...
        <div className="px-3.5 pt-4 pb-3">
          ...
          <div className="space-y-px">
            {viewItems
              .filter(
                (item) => !["chat", "channels", "council"].includes(item.id),
              )
              .map((item) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex w-full items-center gap-2.5 rounded-lg border px-2.5 py-2 text-left transition-all duration-150",
                      isActive
                        ? "border-[#2f4a67] bg-[linear-gradient(90deg,rgba(59,130,246,0.16),rgba(59,130,246,0.03))] text-[#eef5fc]"
                        : "border-transparent text-[#c9d1d9] hover:border-white/[0.06] hover:bg-[#1a212b]",
                    )}
      ...
      {/* Sidebar — Footer */}
      <div className="border-t border-white/[0.07] bg-[#0e141d] px-3.5 py-3">
        <div className="grid grid-cols-3 gap-1.5">
          <button
            onClick={() => setWorkspaceView("activity")}
            className="flex items-center justify-center gap-1.5 rounded-md border border-transparent px-2 py-1.5 text-[11px] text-[#8fa1b3] transition-colors hover:border-white/[0.08] hover:bg-white/[0.04] hover:text-[#e6edf3]"
          >
